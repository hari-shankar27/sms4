...
if($_SERVER["REQUEST_METHOD"]==="POST"){
    ...
    if($name=="" || $email=="" || $password == "" || $teacher_id==""){
        $message="Name, email, teacher id and password are required";
    }
     elseif($password !== $conpass){
        ...
        $stmtcheck=$conn->prepare("SELECT id FROM teachers WHERE email=? OR id=? ");
        $stmtcheck->bind_param("si", $email, $teacher_id);
        $stmtcheck->execute();

        $check=$stmtcheck->get_result();

        if($check->num_rows==0){
            $message="Email and teacher id do match to our records.";
        }
        else{
$hashpassword=password_hash($password, PASSWORD_DEFAULT);

            $stmt=$conn->prepare("INSERT INTO users (name, email, phone, teacher_id, password,role, status) VALUES(?, ?, ?, ?,?, 'teacher', ?)");
            $stmt->bind_param("ssssss", $name, $email, $phone, $teacher_id, $hashpassword, $status);
            if($stmt->execute()){
...
<div class="edit-student">
    ...
    <div class="edithead">
        ...
        <p>Create teacher information</p>
    </div>
    ...
    <div class="edit-card">
       ...
       <?php endif; ?>
        ...
        <form method="POST">
           ...
           <div class="fg">
            <label for="teacher_d">Teacher ID</label>
            <input type="tel" name="teacher_id"  required>
           </div>
