...
$stmt=$conn->prepare("SELECT id, name, email, phone, status FROM users WHERE id=? AND role='teacher' ");
...
$stmt=$conn->prepare("UPDATE users SET name=?, email=?, phone=?, status=? WHERE id=? AND role='teacher' ");
       ...
       <?php endif; ?>
        ...
        <form method="POST">
           ...
           <div class="fg">
            ...
            <input type="text" name="email" value="<?= htmlspecialchars($teacher["email"]) ?>" required>
           </div>
